...
const document_import_1 = require("./file-imports/document-import");
const schema_import_1 = require("./file-imports/schema-import");
...
const processLoaderForSources = memoize_1.default(async (sourceFullPath, sourceContent, addDependency, cwd, options) => {
    ...
    const { schemaHash } = await schema_import_1.appendFileSchemaContext(execContext, codegenContext);
...
const processLoaderForDocuments = memoize_1.default(async (gqlFullPath, gqlContent, addDependency, cwd, options) => {
    ...
    if (fileSchemaContext)
        addDependency(fileSchemaContext.gqlFullPath);
    const fileCodegenContext = [];
    await document_import_1.appendFileContext(execContext, schemaHash, fileCodegenContext, [
        graphqlRelPath,
    ]);
    ...
    const [{ content }] = await codegen_1.processCodegenForContext(execContext, [
        fileContext,
    ]);
    if (!silent)
        print_1.updateLog(`Generating d.ts for ${graphqlRelPath}...`);
    await dts_1.processDtsForContext(execContext, [fileContext]);
    if (!silent) {
